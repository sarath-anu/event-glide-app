...
const EventCardSupabase = ({ event }: EventCardProps) => {
  ...
  return (
    <Card className="event-card group overflow-hidden border-2 border-orange-200 hover:border-orange-400 transition-all duration-300 hover:shadow-lg">
      ...
      <CardHeader className="p-4 pb-2">
        <Link to={`/events/${event.id}`} className="hover:underline">
          <h3 className="text-lg font-semibold line-clamp-2 text-purple-800">{event.name}</h3>
        </Link>
      ...
      <CardFooter className="p-4 pt-0 flex justify-between">
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1">
            ...
            <span className="text-sm">{Math.max(0, event.likes || 0)}</span>
          </div>
          <div className="flex items-center justify-center rounded-full bg-green-100 border border-green-300 p-1">
